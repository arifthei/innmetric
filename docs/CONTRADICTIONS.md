...
Updated 2026-09-12 for Mert's production cutover. Current decisions live in [INTENT.md](INTENT.md); Git procedure lives in [AGENTS.md](../AGENTS.md). This table replaces stacked chronological overrides. Historical wording remains in Git history.

| Conflict | Current resolution |
| --- | --- |
| Old prohibition on editing preview | Mert authorized implementation and pushes to `preview`. Cutover to `main` is a separate authorized step. |
...
| The form does not send but FAQ/privacy imply it does | Public copy always describes the live form. Submits send only when `CONTEXT` is `production` and `INNMETRIC_FORMS_ENABLED` is true. Localhost and deploy-preview stay non-sending and do not show a fake receipt. `/thanks/` is never a receipt. |
...
The four wheel titles, three offers, three process phases, six FAQ questions, current wordmark and latest founder layout remain. Public copy is production-shaped. The collector still requires `CONTEXT=production` and `INNMETRIC_FORMS_ENABLED=true`. **Not bookable** remains kaki. `main` serves the Next app after the authorized cutover.
