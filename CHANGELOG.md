...
- Security headers moved to `next.config.ts` `headers()` (X-Frame-Options, Referrer-Policy, HSTS with subdomains, Permissions-Policy, COOP) and `X-Powered-By` removed. The live check showed `netlify.toml` headers never reached Next responses. `llms*.txt` carry `X-Robots-Tag: noindex`. `netlify.toml` adds `www` to apex 301s; Netlify's own HTTPS upgrade still runs first for `http://www`.
